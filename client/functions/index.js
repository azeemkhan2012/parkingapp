...
exports.verifyCheckoutSession = onRequest(
  ...
  async (req, res) => {
    try {
      ...
      console.log(
        '[verifyCheckoutSession] Session payment_status:',
        session.payment_status,
      );
      console.log(
        '[verifyCheckoutSession] Payment intent status:',
...
function getAvailabilityFromSpot(spotData) {
  ...
  if (
    ...
    return {
      available:
        typeof spotData.availability_available === 'number'
          ? spotData.availability_available
          : parseInt(spotData.availability_available) || 0,
      total:
        typeof spotData.availability_total === 'number'
  ...
  if (
    spotData.availability?.available !== undefined &&
    spotData.availability?.total !== undefined
  ) {
    return {
      available:
        typeof spotData.availability.available === 'number'
          ? spotData.availability.available
          : parseInt(spotData.availability.available) || 0,
      total:
        typeof spotData.availability.total === 'number'
...
async function sendNotificationsForChanges(changeData) {
  ...
  const notificationPromises = affectedUsers.map(async user => {
    try {
      ...
      if (!tokenDoc.exists) {
        console.log(
          `[sendNotificationsForChanges] No FCM token found for user ${userId}`,
        );
        return;
      }
      ...
      if (!fcmToken) {
        console.log(
          `[sendNotificationsForChanges] FCM token is empty for user ${userId}`,
        );
        return;
      }
      ...
      console.log(
        `[sendNotificationsForChanges] Notification sent to user ${userId}:`,
        response,
      );
      ...
      console.log(
        `[sendNotificationsForChanges] Notification stored in Firestore for user ${userId}`,
      );
    } catch (error) {
      console.error(
        `[sendNotificationsForChanges] Error sending notification to user ${user.userId}:`,
        error,
      );
...
exports.monitorParkingSpotChanges = onDocumentUpdated(
  ...
  async event => {
    try {
      ...
      if (!priceChanged && !availabilityChanged) {
        console.log(
          `[monitorParkingSpotChanges] No price or availability changes detected for spot ${spotId}`,
        );
        return;
      }

      console.log(
        `[monitorParkingSpotChanges] Changes detected for spot ${spotId}:`,
        {
          priceChanged,
      ...
      if (savedSpotsSnapshot.empty) {
        console.log(
          `[monitorParkingSpotChanges] No users have saved spot ${spotId}`,
        );
        return;
      }
      ...
      savedSpotsSnapshot.forEach(doc => {
        ...
        const notificationsEnabled =
          savedSpotData.notifications_enabled !== false;
      ...
      if (affectedUsers.length === 0) {
        console.log(
          `[monitorParkingSpotChanges] No users with notifications enabled for spot ${spotId}`,
        );
        return;
      }

      console.log(
        `[monitorParkingSpotChanges] Found ${affectedUsers.length} users to notify for spot ${spotId}`,
      );

      // Prepare change data for notification
      const spotName =
        afterData.name ||
        afterData.location?.name ||
        afterData.title ||
        'Parking Spot';
      const changeData = {
        spotId,
      ...
      console.log(
        '[monitorParkingSpotChanges] Notifications sent successfully',
      );
...
exports.updatePricingHTTP = onRequest(
  ...
  async (req, res) => {
    try {
      ...
      console.log(
        `[updatePricing] Starting update (dryRun: ${dryRun}, minPrice: ${minPrice})...`,
      );
        ...
        try {
          ...
          if (hourlyPrice < minPrice) {
            ...
            const hasNestedFields =
              spotData.original_data?.pricing?.hourly !== undefined;

            if (hasFlatFields && hasNestedFields) {
              // Both formats exist - check if they match
              const flatPrice =
                typeof spotData.pricing_hourly === 'number'
                  ? spotData.pricing_hourly
                  : parseFloat(spotData.pricing_hourly) || null;
              const nestedPrice =
                typeof spotData.original_data.pricing.hourly === 'number'
                  ? spotData.original_data.pricing.hourly
                  : parseFloat(spotData.original_data.pricing.hourly) || null;
              ...
              if (
                flatPrice !== null &&
                nestedPrice !== null &&
                flatPrice !== nestedPrice
              ) {
                needsUpdate = true;
                // Use flat field as source of truth (since app prefers it)
                targetPrice =
                  flatPrice >= minPrice ? flatPrice : getNewPrice(flatPrice);
                console.log(
                  `[updatePricing] Syncing mismatched prices for ${doc.id}: flat=${flatPrice}, nested=${nestedPrice}, will set to ${targetPrice}`,
                );
              }
            }
          }
          ...
          let sourceDailyPrice =
            spotData.pricing_daily || spotData.original_data?.pricing?.daily;
          let sourceHourlyPrice =
            spotData.pricing_hourly ||
            spotData.original_data?.pricing?.hourly ||
            hourlyPrice;
          ...
          if (hasFlatFields) {
            ...
            if (
              newDailyPrice !== null &&
              spotData.pricing_daily !== undefined
            ) {
          ...
          if (hasNestedFields) {
            ...
            if (
              newDailyPrice !== null &&
              originalData.pricing?.daily !== undefined
            ) {
              nestedPricing.daily = newDailyPrice;
            }
            // Preserve other nested fields
            if (originalData.pricing?.currency)
              nestedPricing.currency = originalData.pricing.currency;
            if (originalData.pricing?.freeFor !== undefined)
              nestedPricing.freeFor = originalData.pricing.freeFor;
          ...
          if (!dryRun) {
            ...
            if (batchCount >= BATCH_SIZE) {
              await batch.commit();
              console.log(
                `[updatePricing] Committed batch of ${batchCount} updates. Total updated so far: ${results.updated}`,
              );
          ...
          if (totalProcessed % 100 === 0) {
            console.log(
              `[updatePricing] Processed ${totalProcessed}/${spotsSnapshot.size} spots`,
            );
          }
        } catch (error) {
          console.error(
            `[updatePricing] Error updating spot ${doc.id}:`,
            error,
          );
          results.errors++;
      ...
      if (!dryRun && batchCount > 0) {
        await batch.commit();
        console.log(
          `[updatePricing] Committed final batch of ${batchCount} updates`,
        );
      }
      ...
      const responseResults = {
        ...
        note:
          results.updates.length > 50
            ? `${
                results.updates.length - 50
              } more updates not shown in response`
            : undefined,
      };
...
exports.verifyPricing = onCall(async request => {
  try {
    ...
    console.log(
      `[verifyPricing] Starting verification (minPrice: ${minPrice})...`,
    );
